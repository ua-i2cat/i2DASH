...
i2DASHError i2dash_fragment_setup(i2DASHContext *context)
{
    ...
    if(gf_isom_setup_track_fragment(file, 1, 1, 1, 0, 0, 0, 0) != GF_OK) {
        i2dash_debug_err("gf_isom_avc_set_inband_config: %s",
                                gf_error_to_string(ret));
        return i2DASH_ERROR;
    }

    if (gf_isom_finalize_for_fragment(file, 1) != GF_OK) {
                          ...
                          int buf_len, int dts, int key_frame)
{
    ...
    if(context->frame_number % context->frames_per_fragment == 0){
        ...
        GF_Err ret = gf_isom_set_traf_base_media_decode_time(context->file, 1,
...
i2DASHError i2dash_fragment_close(i2DASHContext *context)
{
    ...
    if(context->frame_number % context->frames_per_fragment ==
        ...
        if(gf_isom_flush_fragments(context->file, 1) != GF_OK) {
